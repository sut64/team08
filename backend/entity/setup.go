...
func SetupDatabase() {
	...
	db.Model(&Illness{}).Create(&unconscious)

	stomachache := Illness{
		Value: "ปวดท้อง",
	}
	db.Model(&Illness{}).Create(&stomachache)

	wounded := Illness{
		Value: "มีแผลสาหัส",
	}
	db.Model(&Illness{}).Create(&wounded)

	bone := Illness{
		Value: "กระดูกผิดปกติ",
	}
	db.Model(&Illness{}).Create(&bone)

	suffer := Illness{
		Value: "มีอาการเจ็บและปวดตามร่างกายอย่างทุกข์ทรมาน",
	}
	db.Model(&Illness{}).Create(&suffer)
